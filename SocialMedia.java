    ...
    public static void register(){
       ...
       passwordField2.setBounds(125, 200, 200, 25);//sets size and position of the textField
        ...
        btnSignUp.addActionListener(new ActionListener() {//The things that happen when the user clicks the button 
            ...
            public void actionPerformed(ActionEvent e) {
               //Declaration
               String registerUsername,registerPassword,cellphone;
                //using the gettext function to get input
                 registerPassword= passwordField2.getText();
                 ...
                 if (registerPassword.length() ==0 && registerUsername.length() ==0 && cellphone.length() ==0){
                      JOptionPane.showMessageDialog(null, "Error please enter username,cellphone number and password  ");
                  }
                 lblMessage2.setText(registerUser(registerUsername,registerPassword,cellphone));
		...
		if(checkCellPhoneNumber(cellphone)== false){
			JOptionPane.showMessageDialog(null,"Cell phone number incorrectly formatted or does not contain the \n South African cellphone code.");//printd mrssage
                //if statement helps to validate the user input
                  if (checkCellPhoneNumber(cellphone)== true && checkUserName(registerUsername)== true && checkPasswordComplexity(registerPassword) == true){
                     
                     //populating the parallel array
                    cellPhone[index] = cellphone;
                    username[index] = registerUsername;
                    password[index] = registerPassword;
                    index++;
                }
       ...
       usernameField.setBounds(125, 100, 200, 25);//sets size and position of the textField
        ...
        btnSignIn.addActionListener(new ActionListener() {//The things that happen when the user clicks the button 
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                password = passwordField.getText();// gets text from the textField
                loginUser(loginUsername, password);// calls the function  
		...
		if(checkPasswordComplexity(password) == false){
			...
			"contains at least eight characters, a capital and small letter, a number, and a special character.");//prints message
                ...
                for (int counter =0; counter < username.length; counter++){
                    if (username[counter] == loginUsername){
                        if (loginUser(loginUsername, password) ==true){
                        JOptionPane.showMessageDialog(null, "Welcome to ConnectCircle \n"+username+" Connect with Your Circle");//prints message
                        break;
                     }//close if   
                    } //close if
                } // close for loo[
                 lblMessage.setText(returnLoginStatus(loginUsername, password));//Sets the text in the label and calls the function
